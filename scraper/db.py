...
# per-broker key (account_oid on idnes, broker_id on most, id on mmreality), the
# contacts, and the firm key (agency_name on idnes, agency_slug on
# ceskereality/remax, agency_id on realitymix). An allowlist, not a hash of the
# whole block, so a parser adding an incidental field can't churn the queue.
