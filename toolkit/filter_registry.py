    ...
    entries: list[FilterDef] = [
        ...
        FilterDef(
            ...
            description=(
                ...
                "remax, realitymix. The semantic is IS LISTED ON: a property "
                "matches if ANY of its listings is from a portal in the list, "
                "so a property advertised on several portals matches under each "
                "of them. Empty list / null = all portals. NOTE for query "
                "builders: `pg_column` is the LISTING-grain answer and is only "
                "correct against a listing-grain relation. On the property-grain "
                "Browse read models `source` is the trust-ranked representative "
                "child's portal, which is a different question and hides 10-22 "
                "percent of each portal's properties; those relations carry "
                "`all_sources` / `active_sources` arrays instead (migration 371) "
                "and must be matched with array overlap, not equality."
            ),
            category=CATEGORY_PROPERTY,
            ui_control=UiControl.MULTISELECT,
            agendas=_ALL_AGENDAS,
