...
There is no third, dark arm any more: W1-b deleted the contract shadow flag and
the `location_claims_shadow` relation `score_shadow_claims` read, so every
projected contract is live and `score_sample` measures all four floors.
...
def _block(
    ...
    """One field's precision/yield block: the same arithmetic on the same floors for
    every field, so no two of them can disagree about which denominator is which."""
