...
`/brokers/{broker_id}` read routes), bearer-gated — mutating operator actions
(merge / dismiss / unmerge). Thin HTTP layer over `api.broker_review`. Reversible:
every merge logs to broker_merge_events.
"""
