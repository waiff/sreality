...
function brokerListing(over: Partial<brokers.BrokerListing>): brokers.BrokerListing {
  return {
    listing_id: 1,
    sreality_id: null,
    property_id: null,
    ...
    ...over,
  } as unknown as brokers.BrokerListing;
}
...
describe('<BrokerDetail> locality cell never fabricates a listing link', () => {
  it('renders inert text when the row has no resolvable destination', async () => {
    vi.mocked(brokers.fetchBrokerListings).mockResolvedValue([
      brokerListing({ listing_id: 1, locality: 'Bez cile' }),
    ]);
  ...
  it('still links a row that carries a property id', async () => {
    vi.mocked(brokers.fetchBrokerListings).mockResolvedValue([
      brokerListing({ listing_id: 2, property_id: 42, locality: 'S cilem' }),
    ]);
    renderPage();
  ...
  it('still links a row that carries a sreality id', async () => {
    vi.mocked(brokers.fetchBrokerListings).mockResolvedValue([
      brokerListing({ listing_id: 3, sreality_id: -284913, locality: 'Legacy' }),
    ]);
    renderPage();
