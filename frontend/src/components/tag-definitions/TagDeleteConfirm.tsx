 ...
 * scroll lock come from <Dialog> (lib/useDialog.ts). Two properties are this
 * component's own and stay: a delete already in flight is not dismissible (so
 * `pending` gates the one close path Escape and the backdrop now share), and
 * initial focus goes to CANCEL rather than to the first focusable control. */
interface Props {
...
export default function TagDeleteConfirm({
  ...
  /* APG: a destructive confirm opens on its LEAST destructive control —
   * Cancel, not the acknowledgement checkbox DOM order would pick. */
  const cancelRef = useRef<HTMLButtonElement>(null);

  return (
    <Dialog
      open
      /* The ONE close path: Escape and a backdrop press both land here, and
       * neither may dismiss a delete that is already in flight. */
      onClose={() => {
        if (!pending) onCancel();
      }}
      label="Delete tag"
