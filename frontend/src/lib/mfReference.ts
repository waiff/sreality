 ...
 * town level in a town priced per katastr. `per_m2_*` are TOTAL rates (the
 * town's published span of the reference flat's rate plus this flat's
 * adjustments), the range twin of `total_per_m2`, so `rent_*_czk` = per-m² ×
 * area. The yields exist only for a sale flat with a usable price. */
