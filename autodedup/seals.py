...
SPENT_SEALS: dict[str, str] = {
    ...
    "12000c922a0e1a3204028cfcd06cbddc312acc4ec2bcfefe4638956597670938": (
        "W16 phase 4's confirmation seal: the W1 cohort's cluster map (4,882 listings, 661 "
        "groups) under a fresh seed 20260938, cut from the merge edges AND the cluster "
        "memberships of SIX arms — g7 (settings/w13.json), g8 (w14), g8b (w15) and the W16 "
        "ladder L, M, S (w16_l, w16_m, w16_s) — because those three re-partition (E156) and "
        "demote on promotion (E157/E158) and so are not sub-partitions of g8b's. The map came "
        "back BYTE-IDENTICAL to f30684fc…'s: S ⊆ M ⊆ L ⊆ g8b on merged pairs and each arm's "
        "clusters refine g8b's, so this cohort's component map is a fixpoint under the whole "
        "ladder and the only fresh holdout available over it is a fresh SEED (M108's finding, "
        "a third time). Structural NEGATIVES are not unioned, for the measured reason "
        "e1d8bb98… recorded. SPENT by ONE read of g7, g8b, L, M and S (w14/confirm_seal4/): "
        "of 201 sealed labelled duplicates L holds 165, g8b 165, M 159 and S 156 against g7's "
        "105 (McNemar vs g7 — L gained 66 lost 6, p ~ 7.3e-14; M 60/6, p ~ 2.7e-12; S 57/6, "
        "p ~ 1.6e-11; independent tiers alone 96 / 96 / 95 / 95 of 113 against 84). Criterion "
        "(i) HOLDS for L, M and S under all three predicates — w14, w15 and the wider w16 "
        "(= w16_l, whose reader flags all three carry): 0 independent-tier pairs carrying a "
        "fact, 0 bad groups, 0 operator must-not-link. It FAILS for g8b, whose two sealed "
        "groups fuse three flipped negatives that the W16 body_align reader separates and no "
        "earlier reader does. Criterion (ii) holds for every arm (operator-tier losses vs g7, "
        "whole corpus: g8b 22, L 24, M 24, S 23, limit 30) and (iv) holds for L, M and S (20 "
        "shuffles of decision AND row order, one partition each; w13/w14/w15 replay g7/g8/g8b "
        "member for member). The whole-cohort group stress that followed then REFUSED L: it "
        "still co-clusters 2 of the hand-read E145 two-unit pairs — 76044x274541 (Harfa "
        "Living, the 2. NP unit against the 3. NP one) and 82912x83291 (Kolbenova 394 m² "
        "offices, 4./5 against 3. NP) — and no reader of any predicate reaches either, which "
        "is exactly the fail-open D50 was written against; M and S co-cluster none of the "
        "five. The next choice needs a fresh seed again."
    ),
