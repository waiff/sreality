-- 162_listings_obec_id_growth_filter.sql
--
-- Capture each listing's obec_id (= admin_boundaries.id, the RÚIAN obec code) so
-- Browse can filter listings by per-obec price-stats growth. The growth RPC
-- (price_stat_growth) already keys on admin_boundaries.id; the listings admin-geo
-- trigger (migration 140) already does the exact st_covers PIP into the same
-- table to derive obec/okres/region NAMES -- it just discarded ob.id. Capturing
-- it turns the growth filter into a cheap integer match (no per-query spatial
-- scan, anon 3s timeout unaffected).
--
--   1. listings.obec_id (plain bigint, no FK -- matches obec/okres/region) + index
--   2. extend listings_set_admin_geo() to set new.obec_id from the same PIP
--   3. surface obec_id on listings_public + properties_public (trailing)
--   4. browse_stats_properties gains obec_ids_filter bigint[] so Stats stays
--      aligned with Map/Table when the growth filter is active
--
-- Existing rows are backfilled out-of-band (set-based, batched) right after this
-- migration -- same pattern migration 140 used for the obec/okres/region names.
...
comment on column listings.obec_id is
  'Municipality (obec) RÚIAN code = admin_boundaries.id, derived from geom via the '
  'same PIP that fills obec/okres/region. Joins to price_stat_growth.obec_id.';

create index if not exists listings_obec_id_idx
  on listings (obec_id) where obec_id is not null;


-- 2. trigger: also capture ob.id ---------------------------------------------
...
begin
  ...
  -- obec_id so a re-detail-fetch of a not-yet-backfilled row self-heals.
  if tg_op = 'UPDATE'
     and new.geom is not distinct from old.geom
     and new.okres is not null
     and new.obec_id is not null then
    return new;
  end if;

  select ob.id, ob.name, ok.name, kr.name
    into v_obec_id, v_obec, v_okres, v_kraj
...
-- 3a. listings_public: expose obec_id (trailing) ------------------------------
...
-- 3b. properties_public: expose the repr listing's obec_id (trailing) ---------
...
-- 4. browse_stats_properties: add obec_ids_filter -----------------------------
-- Reproduced VERBATIM from migration 159; the ONLY changes are (1) the trailing
-- obec_ids_filter param and (2) one WHERE clause. The signature changes, so the
-- exact current (159) overload is DROPped first.
