-- 203: retire the listing-grain curation tables, now that the property-grain
-- ones (migration 202) replace them.
--
-- DESTRUCTIVE (DROP TABLE) but the three tables are EMPTY in production
-- (verified 0 rows) and never held history — the curation feature is unused, so
-- nothing is lost. Order inside this single transaction is safe-by-construction:
-- browse_stats_properties is repointed to property_tags BEFORE listing_tags is
-- dropped, so no live reader is ever left referencing a dropped relation.

-- (1) browse_stats_properties: reproduced VERBATIM from migration 182; the ONLY
--     change is the tag subquery — listing_tags-by-sreality_id becomes
--     property_tags-by-property_id (the Browse tag filter is now dedup-stable:
--     it matches a property if ANY of its listings' property carries the tags,
--     i.e. directly at property grain rather than via the representative listing).
--     Body-only change, identical signature, so CREATE OR REPLACE suffices (no
--     exact-signature DROP, no PostgREST schema-cache / deploy-window concern).
...
-- (2) drop the listing-grain curation surface (views first, then RPC, then the
--     now-unreferenced tables). All empty; no data lost.
drop view if exists collection_listings_public;
drop view if exists listing_tags_public;
drop view if exists listing_notes_public;
drop function if exists listings_with_tags(bigint[]);
drop table if exists collection_listings;
drop table if exists listing_tags;
drop table if exists listing_notes;
