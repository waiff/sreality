-- 112_health_checks_fetch_failures_fix.sql
--
-- Fix the fetch_failures count in scraper_health_checks(p_source) from
-- migration 111. The inner join to listings_public dropped failure rows whose
-- listing was never created (the very first detail fetch failed, so no row
-- exists) — undercounting sreality's backlog from 1494 to <1000 and masking
-- the WATCH signal the operator relies on.
--
-- Fix: LEFT JOIN and coalesce a missing source to 'sreality'. Orphan failures
-- (no listing row) count toward sreality — the only portal that writes
-- listing_fetch_failures today (bazos/bezrealitky record_failure is a no-op).
-- Matched rows use their listing's real source, so this stays source-generic.
-- Only the two fetch_failures subqueries change; everything else is identical
-- to migration 111.
