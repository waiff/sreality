-- 088_scraper_health_checks.sql
-- Operator-facing scraper health checks for the Health page.
--
-- A single SECURITY INVOKER function computes pass/warn/fail for each check
-- live from the existing *_public views — no new tables, no writes, no
-- schema changes to existing objects. Same anon-read pattern as
-- health_summary (013) and recent_scrape_runs (087): anon already has SELECT
-- on scrape_runs_public / listings_public / listing_fetch_failures_public /
-- images_public, so an invoker-rights function reading only those is safe.
--
-- The Health page calls this on its 60s refetch so the dashboard reflects
-- current scraper behaviour without any background job.
...
with
runs24 as (
  select * from scrape_runs_public where started_at > now() - interval '24 hours'
),
m as (
  select
    extract(epoch from now() - (select max(started_at) from scrape_runs_public))/60.0 as mins_since_start,
    (select max(started_at) from scrape_runs_public) as last_start,
...
select jsonb_build_object(
  ...
  'checks', jsonb_build_array(
    jsonb_build_object(
      ...
      'status', case when mins_since_start < 40 then 'pass'
                     when mins_since_start < 90 then 'warn' else 'fail' end,
      'value', coalesce(round(mins_since_start::numeric, 0)::text, '–') || ' min ago',
      'detail', 'Last run started ' || coalesce(to_char(last_start, 'YYYY-MM-DD HH24:MI'), 'never')
                || ' UTC. The 15-min delta should keep this low. Warn >40 min, fail >90 min.'
    ),
    jsonb_build_object(
