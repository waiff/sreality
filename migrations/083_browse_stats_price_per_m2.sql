-- 083_browse_stats_price_per_m2.sql
--
-- Phase QUAL — surface price-per-m² bounds on every analytical surface.
-- The Browse Map / Table / Cards already filter through PostgREST
-- against `listings_public.price_per_m2` (the computed numeric column
-- exposed by the view) once the new registry entries are wired in.
-- The Stats tab calls `browse_stats` directly, so until this migration
-- its counts ignored the new clauses — Stats would have shown a wider
-- cohort than Map / Table whenever a per-m² bound was set.
--
-- This migration extends the 44-param `browse_stats` from migration 080
-- to 46 params, adding:
--   price_per_m2_min double precision
--   price_per_m2_max double precision
--
-- Predicate uses the same `price_czk::numeric / NULLIF(area_m2, 0)`
-- expression as the toolkit's `_shared_filter_where` and the matcher's
-- `_build_match_clauses`, so SQL paths agree on the definition.
-- Listings with NULL area_m2 fall out when either bound is set —
-- intentional, matches the registry's documented semantics.
--
-- Same DROP-then-CREATE pattern migration 080 uses: adding params
-- creates a new overload, and named-argument callers become ambiguous
-- against the prior 44-param version. The DROP removes the old
-- overload; grants come from the role-level `GRANT EXECUTE ... TO anon`
...
create or replace function browse_stats(
  ...
  price_per_m2_min         double precision default null,
  price_per_m2_max         double precision default null
)
returns jsonb
language sql
stable
