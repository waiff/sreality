-- 109_browse_stats_force_custom_plan.sql
--
-- Fix: Browse > Stats (browse_stats_properties) times out
-- ("canceling statement due to statement timeout") on large / unfiltered
-- cohorts, even though the same query with literal filter values runs in
-- ~85 ms.
--
-- Root cause: the function takes ~50 optional, null-guarded filter params
-- and is invoked by PostgREST with bind parameters. As a `language sql`
-- function it is NOT inlined (its body references properties_public plus
-- the heavy correlated EXISTS subqueries for districts / city-quality /
-- city-proximity), so its body is planned ONCE as a generic plan with the
-- 50 args as parameters. Under that generic plan Postgres cannot fold away
-- `(<param> is null or col = <param>)` guards: it can't use
-- properties_category_idx and keeps the per-row st_dwithin city checks in
-- the plan. Over ~78k properties that blows past the statement timeout.
-- With the actual values known (custom plan) every unused branch is pruned
-- and the same body returns in tens of ms.
--
-- `plan_cache_mode` does NOT govern a non-inlined `language sql` body, but
-- it DOES govern PL/pgSQL. So: re-create the function as `language plpgsql`
-- with `SET plan_cache_mode = 'force_custom_plan'`. The embedded query is
-- then re-planned with the real argument values on every call -> the null
-- guards fold, the category index is used, and the city EXISTS subqueries
-- drop out when their filters are unset.
--
-- Body is byte-for-byte the migration 095 query, only wrapped in
-- `RETURN ( ... );`. Signature, argument names/defaults, return type, and
-- grants are unchanged (CREATE OR REPLACE preserves privileges), so the
-- frontend contract is identical -- this is purely a planner fix.
--
-- Validated before shipping: a PL/pgSQL copy carrying this body + setting,
-- exercised through a force_generic_plan prepared statement (worst case),
-- returned in ~67 ms vs the >25 s timeout.
