-- 537_browse_hides_dismissed.sql
-- Browse hides the caller's dismissed properties (migration 536) by default,
-- server-side, on every surface: cards, table, count, map pins, map cells, Stats.
--
-- WHY THE EXCLUSION LIVES HERE, NOT IN THE CLIENT. Every Browse prefilter today
-- is an id ALLOWLIST the SPA resolves and sends as `.in(...)` in the GET URL. A
-- dismissed set is the opposite shape (an exclusion) and grows without bound --
-- the operator's trash pile -- so `.not.in(<ids>)` would outgrow the URL, and
-- `.not.in.()` on an empty set is a PostgREST syntax error. The caller's own RLS
-- already knows the set: `property_dismissals_public` (536) is the ONE read
-- definition of "dismissed for me", and every predicate below reads it.
--
-- WHY FUNCTIONS, NOT VIEWS. `browse_list` and `properties_map_mv` are rebuilt
-- blue-green every tick (DROP + rename, migrations 522/440): a view or an RLS
-- policy on them would either block the DROP or vanish with it. A LANGUAGE sql
-- function body is not dependency-tracked, so the swap is untouched, and a
-- set-returning SQL function that is STABLE, SECURITY INVOKER and carries no
-- SET clause is INLINED by the planner -- PostgREST's filters, ORDER BY and
-- LIMIT reach the underlying index exactly as they do today. The return type is
-- `browse_projection`'s row type (both relations are `select * from` it), NOT
-- the relation's own, which would pin the blue-green DROP. A migration that
-- drops `browse_projection` without CASCADE now fails loudly on these two.
--
-- Measured on production (rolled-back probe, 20,000 dismissals stacked on the
-- NEWEST rows of the default byt+pronajem cohort, the worst case): card page
-- 48.7 ms (parameterized anti-join probes), exact count 115 ms vs 31 ms
-- baseline, an unrelated cohort 0.19 ms. NOT EXISTS over NOT IN: no hash-memory
-- cliff, and the same idiom the Stats RPC already uses for with_estimates.
--
-- The two aggregate RPCs cannot take a function in place of their relation
-- without losing "show dismissed", and Stats/map must never describe a
-- different cohort than the list beside them (migrations 351/378). So each gets
-- ONE trailing parameter, `hide_dismissed` (default false -- every existing
-- caller keeps today's answer), and ONE clause after `property_ids_filter`.
-- A parameter list cannot change under CREATE OR REPLACE, so each is dropped by
-- its exact live signature and recreated with migration 504's body, verified
-- byte-identical to the live `prosrc` (md5 cbd4a9b6... / 90d7a030...) before
-- this file was generated; the ACL is restored to exactly what it was
-- (authenticated + service_role; never public or anon).
--
-- ADDITIVE in effect, one transaction. APPLY BEFORE MERGING: the SPA that ships
-- with this file calls the three functions and passes `hide_dismissed`.

begin;

set local lock_timeout = '5s';

-- 1. The three dismissal-aware sources, one per relation Browse reads.
...
create or replace function public.listing_feed_visible()
returns setof public.listing_feed_public
language sql
stable
as $$
  select f.* from public.listing_feed_public f
  where not exists (
    select 1 from public.property_dismissals_public d where d.property_id = f.property_id)
$$;
...
revoke execute on function public.listing_feed_visible() from public, anon;
...
grant execute on function public.listing_feed_visible() to authenticated, service_role;

-- 2a. browse_stats_properties -- migration 504's body, one clause and one parameter added.
drop function if exists public.browse_stats_properties(
  ...
  double precision, text[], text[], jsonb, integer, integer, jsonb,
  ...
  boolean[], text[], integer, integer, bigint[], text[], bigint[], integer,
  integer, integer, integer, integer, integer, double precision, boolean,
  boolean, bigint[]
);
...
-- 2b. browse_map_cells -- migration 504's body, one clause and one parameter added.
...
revoke execute on function public.browse_map_cells(
  text[], text[], integer, integer, integer, integer, boolean, integer,
  integer, integer, integer, integer, integer, boolean, boolean, boolean,
  boolean, text[], boolean, boolean, boolean, integer, text[], bigint[],
  text[], text, double precision, double precision, double precision,
  ...
  boolean[], text[], integer, integer, bigint[], text[], bigint[], integer,
  integer, integer, integer, integer, integer, double precision, boolean,
  boolean, bigint[], bigint[], integer, boolean
) from public, anon;
grant execute on function public.browse_map_cells(
  text[], text[], integer, integer, integer, integer, boolean, integer,
  integer, integer, integer, integer, integer, boolean, boolean, boolean,
  boolean, text[], boolean, boolean, boolean, integer, text[], bigint[],
  text[], text, double precision, double precision, double precision,
