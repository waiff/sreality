-- 037_browse_stats_categories.sql
-- Extend browse_stats() with category_main_filter and category_type_filter
-- so the Browse page's new category picker (Byty / Domy / Komerční ×
-- Pronájem / Prodej) flows through to the Stats tab. Without it the
-- map/table cohorts would narrow as the operator picks a category but
-- the Stats tab would silently aggregate across every category — a
-- confusing inconsistency.
--
-- DROP-then-CREATE matches the pattern set by 024_browse_stats_filters
-- and 033_browse_stats_tag_ids. Postgres identifies a function by full
-- argument signature, so adding parameters would otherwise leave a
-- second overload behind and the SPA's by-name calls would become
-- ambiguous.
--
-- NULL or omitted category filters skip the predicate so any existing
-- caller (e.g. region_stats internals, ad-hoc SQL) keeps working.
