...
--
-- The view keeps its columns; it now shows the ingest queue only. Presence
-- checks are visible through the new `listing_presence_checks_public`.
-- `create or replace view` keeps the existing grant (anon select, migration 109).
...
grant select on listing_presence_checks_public to anon;
