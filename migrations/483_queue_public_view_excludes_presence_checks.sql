...
-- not see into listing_detail_queue at priority -2 (QUEUE_PRIORITY_VERIFY; -1 is
-- the location-data refetch lane, migration 384), and
...
--
-- The view keeps its columns and its admin gate (migration 318; the gate opens
-- for the claims-less pg_cron connection that refreshes the matview, migration
-- 330); it now shows the ingest queue only.
...
create or replace view public.listing_presence_checks_public as
select * from (
...
comment on view listing_detail_queue_public is
  'The ingest queue (new, changed, failure-retry, location refetch). Presence checks (priority -2) live in listing_presence_checks_public — they are served last by design and must not count as ingest lag (rule #3, 2026-09-07).';
