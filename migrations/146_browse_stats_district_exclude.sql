-- 146_browse_stats_district_exclude.sql
--
-- Keep the Stats tab in lockstep with Map / Table + the Watchdog matcher
-- (architectural rule 16) for the new district EXCLUDE feature: a district
-- chip can now be flagged `excluded` to SUBTRACT its matches from the cohort
-- instead of requiring them. Browse Map/Table (queries.ts) build the same
-- include/exclude split as `and(or(<include chips>), not.or(<exclude chips>))`,
-- and the Watchdog matcher (`api/notifications._build_match_clauses`) appends a
-- `NOT (...)` group — this migration mirrors that in the Stats RPC.
--
-- New param `districts_excluded_filter boolean[]` is parallel to
-- `districts_filter` / `districts_context_filter` (one flag per chip). NULL or
-- a NULL element coalesces to false (= include), so every existing caller is
-- unchanged. The district predicate becomes two gates:
--   INCLUDE: no include-chips OR the row matches at least one include-chip
--   EXCLUDE: the row matches NO exclude-chip
--
-- Adding a param changes the signature, so DROP + CREATE (CREATE OR REPLACE
-- can't add arguments). The new param defaults NULL and is appended last.
...
drop function if exists public.browse_stats_properties(
  ...
  boolean, boolean, boolean, integer, text[], bigint[], text, text, double precision,
  double precision, double precision, double precision, text, double precision,
  double precision, double precision, double precision, integer, double precision,
  double precision, text[], text[], jsonb, integer, integer, jsonb, double precision,
  double precision, integer, integer, integer, double precision, text[],
...
begin
  ...
  with filtered as (
    ...
    where
      ...
      -- District INCLUDE gate: no include-chips, OR the row matches at least
      -- one include-chip (excl flag coalesces to false = include).
      and (
      ...
      -- District EXCLUDE gate: drop rows matching any exclude-chip.
      and (
        districts_filter is null or array_length(districts_filter, 1) is null
        or not exists (
          select 1 from unnest(districts_filter,
      ...
      -- City population now reads the precomputed home_obec_pop column
      -- (migration 142): the listing's own municipality, country-wide. No
      -- curated-city centroid join -> no anon timeout.
      and (city_pop_min is null or l.home_obec_pop >= city_pop_min)
      and (city_pop_max is null or l.home_obec_pop <= city_pop_max)
      -- Fast polygon-edge proximity (migration 142): max metric within a fixed
      -- 5 / 15 km, filtered `>= value`.
      ...
      -- Curated-city index-rule list (any of 33 indexes) keeps the centroid
      -- footprint EXISTS; population is no longer part of it.
