-- 095_property_grain_stats.sql
-- Slice 2a of the multi-portal dedup track: make browse_stats_properties fast
-- enough to back the Stats tab, and add the four derived predicates.
--
-- Root cause of Slice 1's perf gap: properties_public is a join view
-- (properties + the representative listing), and browse_stats_properties is a
-- function whose param-guarded predicates reference the joined listing's
-- filter columns (furnished, condition, locality, ...). Under the function's
-- generic plan those references can't be folded away, so the join is retained
-- and the aggregate runs far slower than the single-table listing-grain
-- browse_stats.
--
-- Fix: denormalize every filter column browse_stats_properties touches onto
-- `properties` (maintained by the recompute job from the representative
-- listing), and re-source them in properties_public from `properties`. The
-- function then references only properties-native columns, so Postgres prunes
-- the listings join entirely -- making it structurally identical to the
-- single-table original. The join stays in the view only for display-only
-- columns (price_unit, floor, broker_*, description, ...) that no Browse
-- aggregate references, so it prunes per-query where unused.
--
-- Also adds the four derived predicates (distinct_site_count_min,
-- price_drop_count_min, price_rise_count_min, max_price_drop_pct_min) on the
-- columns the recompute job already populates (migrations 091 + 093).
--
-- The 16 new columns are backfilled in-migration from the representative
-- listing so there is no window where properties_public returns NULL for them;
-- the recompute job maintains them every run thereafter.
...
alter table properties
  ...
  add column if not exists cellar          boolean,
  add column if not exists garage          boolean,
  add column if not exists category_sub_cb integer,
  add column if not exists estate_area     numeric(9,1),
  add column if not exists usable_area     numeric(9,1),
  add column if not exists garden_area     numeric(9,1),
...
update properties p set
  ...
  cellar          = l.cellar,
  garage          = l.garage,
  category_sub_cb = l.category_sub_cb,
  estate_area     = l.estate_area,
  usable_area     = l.usable_area,
  garden_area     = l.garden_area,
  parking_lots    = l.parking_lots
from listings l
where l.sreality_id = p.repr_listing_id;

-- Re-source the 16 denormalized columns from `properties` (was the joined
-- listing in migration 093). Column names / types / order are unchanged, so
-- this is a valid CREATE OR REPLACE; only the source expressions move from
-- l.* to p.*. The display-only columns still come from the LEFT JOIN.
...
-- browse_stats_properties: same body as migration 094, now (a) fast because
-- properties_public no longer forces the join for the columns it references,
-- and (b) with four new derived predicates appended to the filtered CTE.
-- Drop the 094 (46-arg) signature first: adding the four new params creates a
-- distinct overload rather than replacing it, which would leave the call
-- ambiguous.
drop function if exists public.browse_stats_properties(
  text[],text[],integer,integer,integer,integer,boolean,integer,integer,integer,
  integer,integer,integer,boolean,boolean,boolean,boolean,text,boolean,boolean,
  boolean,integer,text[],bigint[],text,text,double precision,double precision,
  ...
  text[],text[],jsonb,integer,integer,jsonb,double precision,double precision
);
