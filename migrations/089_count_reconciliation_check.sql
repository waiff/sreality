-- 089_count_reconciliation_check.sql
-- Adds a 10th scraper health check: per-category reconciliation of our
-- active-listing count against sreality's reported result_size, surfaced
-- from the latest completed run's by_category (which the region-split
-- scraper now records with sreality_result_size / collected / active_db).
--
-- Redefines scraper_health_checks() from migration 088 (a create-or-replace
-- must restate the whole function) appending one check. Still SECURITY
-- INVOKER reading only *_public views — same anon-read posture as 088.
  ...
  'checks', jsonb_build_array(
    ...
    jsonb_build_object(
      ...
      'detail', 'Largest per-category gap between our active count and sreality''s '
                || 'reported result_size on the latest completed run. Populates once the '
