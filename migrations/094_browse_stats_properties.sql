-- 094_browse_stats_properties.sql
-- Slice 1 of the multi-portal dedup track: property-grain Browse stats.
--
-- A verbatim clone of the current browse_stats (migration 083 lineage) with
-- exactly one change: the `filtered` CTE reads `from properties_public`
-- instead of `from listings_public`. Same argument list, same WHERE shape,
-- same JSON return contract. The four Multi-portal derived predicates
-- (distinct_site_count, price_drop_count, price_rise_count, max_price_drop_pct)
-- land in Slice 2; this slice only moves the existing stats onto the
-- property grain. properties_public exposes sreality_id (= the
-- representative listing) so the tag-id subquery and every other clause
-- carry over unchanged.
--
-- NOT YET WIRED TO THE FRONTEND. The function is created and correct, but
-- it is a heavy aggregate and properties_public is a join view, so under the
-- function's generic plan it runs materially slower than the already-borderline
-- listing-grain browse_stats. Slice 1's fetchBrowseStats deliberately stays on
-- browse_stats; while properties are 1:1 with listings the two return identical
-- numbers. Slice 2 owns the perf pass (precomputed property stats / predicate
-- restructuring) that makes this safe to back the user-facing Stats tab.
