-- 155_browse_stats_subtype.sql
--
-- Add a multi-select `subtype_filter text[]` to browse_stats_properties (the
-- property-grain Browse Stats RPC). Reproduced VERBATIM from the current prod
-- definition (pg_get_functiondef, supersedes migration 146); the ONLY changes
-- are (1) the new trailing param and (2) one WHERE clause after the existing
-- category_sub_cb clause. The clause is a plain indexed-text `= ANY` on the
-- denormalized properties_public.subtype column — no jsonb/spatial scan, so the
-- anon 3s statement-timeout budget is unaffected.
--
...
-- browse_stats() is intentionally NOT touched — it has no live caller (Browse
-- Stats reads browse_stats_properties).
