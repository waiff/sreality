-- 165_browse_stats_furnished_ownership_multi.sql
--
-- Furnished + Ownership become MULTI-SELECT filters with an "Unknown" bucket.
-- browse_stats_properties' furnished_filter / ownership_filter change from
-- `text` (single value, exact match) to `text[]` (a listing matches if its
-- value is in the list). The special '__unknown__' array element matches
-- listings whose value is NULL or stored under a non-canonical label
-- (e.g. legacy remax 'vybaveno' / 'ostatni') -- so the Stats tab stays aligned
-- with Map/Table and the Watchdog matcher, which use the same logic
-- (toolkit.comparables._enum_or_unknown_clause).
--
-- Reproduced VERBATIM from migration 162; the ONLY changes are the two filter
-- param types and their two WHERE clauses. The signature changes (text -> text[]
-- for two params), so the exact current (162) overload is DROPped first.
