-- 278_browse_stats_on_read_model.sql
--
-- Stats tab joins the unified Browse read model (design docs/design/
-- browse-read-model.md, slice 4). browse_stats_properties re-points its single
-- FROM clause from properties_public (the live view: view + listings LEFT JOIN
-- + per-query gate InitPlan over the churned 450k-row OLTP table) to
-- browse_list (migration 276: the compact snapshot rebuilt every 5 min by
-- pg_cron, migration 277). Stats / header count / cards / table now read the
-- SAME relation, so the four surfaces can never disagree — one snapshot, one
-- number. The gate + status='active' predicates live in browse_projection
-- (baked into browse_list rows at rebuild), so the function needs no gate of
-- its own — exactly as before, when properties_public's WHERE carried them.
--
-- The argument signature is UNCHANGED (74 args, migration 223), so this is a
-- body-only CREATE OR REPLACE — no DROP, no PGRST202 window, no frontend
-- change. Body reproduced VERBATIM from migration 223; the ONLY edit is the
-- FROM relation. Every column the body references exists on browse_list —
-- pinned by tests/test_browse_read_path_guardrail.py's read-contract test.
--
-- Semantics note: Stats previously computed over live rows while cards/table
-- (post re-point) read the snapshot; this closes that gap rather than opening
-- one. Freshness of ALL Browse surfaces is now readable off
