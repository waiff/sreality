-- 062_browse_stats_tom_mean.sql
--
-- Adds `mean` to each bucket's `tom_box` in browse_stats' new
-- price_quartile_velocity output. The five-number summary
-- (min, p25, median, p75, max) shipped in 061 hides the price-velocity
-- signal whenever the data window is short: tom_days is integer-floored,
-- and with ~290 listings per quartile the median position lands inside
-- the same integer plateau across all four buckets, producing four
-- visually identical box plots. The mean — being a continuous statistic
-- — separates cleanly even on integer-clumped data.
--
-- Pure additive CREATE OR REPLACE: the signature is unchanged from 061
-- (38 args), so grants survive without a re-grant. Same pattern as 042
-- (which added ppm2_box to the disposition output the same way).
...
as $$
  ...
  price_quartile_velocity as (
    select
      ...
      avg(tom_days) filter (where tom_days is not null)                          as tom_mean
