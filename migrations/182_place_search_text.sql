-- 182: one canonical free-text place column for location-chip matching.
--
-- WHY: portals disagree on where the street lives. sreality embeds it in the
-- free-text `locality` ("Pezinská, Mladá Boleslav ..."); bazos (and the street
-- backfills) store the town in `locality` and the street in the structured
-- `street` column. The street-level location chip ("locality" level) matched
-- only `locality`, so bazos listings were invisible to street picks even when
-- obec, bbox and deal all matched.
--
-- FIX: properties_public gains `place_search_text` = concat_ws(street,
-- locality) — the single definition of "the free-text place words of a row" at
-- the finest grain. Every chip-matching surface (Browse map/table via
-- PostgREST, this RPC for the Stats tab, the watchdog matcher in
-- api/notifications.py) matches that ONE column wherever it consulted
-- `locality` before: the 'locality' (street-pick) branch and the legacy
-- no-level fallback branch alike, so pre-resolution saved chips behave
-- identically. PostgREST can only filter on view columns — a view column is
-- the one mechanism that centralizes the semantic for all three surfaces.
-- Matching-only; not for display (street is the representative listing's).
--
-- browse_stats_properties is a body-only change: the signature is identical
-- to migration 173, so CREATE OR REPLACE suffices (no exact-signature DROP,
-- no PostgREST schema-cache / deploy-window concern).

-- 1. properties_public — reproduced VERBATIM from migration 173; the ONLY
--    change is the trailing place_search_text column.
...
create or replace view properties_public as
 SELECT p.id AS property_id, p.repr_listing_id AS sreality_id, p.first_seen_at,
    ...
    concat_ws(', ', l.street, p.locality) AS place_search_text
   FROM properties p
...
comment on column public.properties_public.place_search_text is
  'Free-text place words for location-chip matching (street + locality, '', ''-joined). Matching-only — not a display field.';


-- 2. browse_stats_properties — reproduced VERBATIM from migration 173; the
--    ONLY change is the district-chip text matching: l.locality ->
--    l.place_search_text in the 'locality' branch and the legacy fallback
--    branch (include and exclude blocks alike).
