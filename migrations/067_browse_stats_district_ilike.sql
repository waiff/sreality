-- 067_browse_stats_district_ilike.sql
--
-- Fix the district filter so picks from the Mapy.cz-powered
-- typeahead at any granularity (kraj, okres, obec, část obce, street,
-- POI) narrow the cohort instead of zeroing it out.
--
-- Before this migration the predicate was `district = ANY(districts_filter)`.
-- `listings.district` only carries canonical okres names without
-- diacritics (e.g. `'okres Jihlava'`, `'Praha'`), so picking
-- `'Hruškové Dvory'` (a `regional.municipality_part`) returned zero
-- even though listings in that neighbourhood exist and their
-- `locality` text reads `'Jihlava - Hruškové Dvory, okres Jihlava,
-- okres Jihlava'`. The frontend now uses the same predicate (an
-- ILIKE substring across `district` OR `locality`, OR'd across
-- chips) — both code paths must stay in lockstep.
--
-- Pure CREATE OR REPLACE: signature is byte-for-byte unchanged from
-- 063, so the existing `grant execute … to anon` survives and no
-- re-grant is needed.
--
-- Performance: ILIKE on `locality` is a seq-scan at the current row
-- count (tens of thousands) and runs well under 100 ms, matching
-- the existing seq-scan profile of `browse_stats`. If row count
-- ever crosses ~250k, add a follow-up migration that runs
-- `create extension if not exists pg_trgm;` and
-- `create index on listings using gin (district gin_trgm_ops, locality gin_trgm_ops);`.
-- Not added speculatively — `pg_trgm` isn't currently in the
-- migration set and adding extensions casually expands the deploy
-- surface.
