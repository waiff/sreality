-- 374_city_quality_browse_list.sql
--
-- NOTE ON NUMBERING: applied live via the Supabase MCP under the name
-- `373_city_quality_browse_list` before a concurrent branch merged
-- `373_new_dedup_labeling` to main. The recorded name is immutable, so the
-- DISK file is renumbered to 374 to clear the duplicate-number guard
-- (mirrors migration 363's identical situation); prod is unaffected
-- (already applied), and CI replays this file on a fresh DB where the
-- CREATE OR REPLACE statements are a clean forward.
--
