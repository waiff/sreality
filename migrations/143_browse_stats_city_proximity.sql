-- 143_browse_stats_city_proximity.sql
--
-- Keep the Stats tab in lockstep with Map / Table (architectural rule 16) for
-- the city-proximity filters reworked in migration 142:
--   * city_pop_min / city_pop_max now read the precomputed `home_obec_pop`
--     column (the listing's OWN municipality population, country-wide) instead
--     of the slow curated-city ST_DWithin(centroid) EXISTS that timed out the
--     anon role — the same bug that made the Min Population filter return zero.
--   * eight new params (near_{pop,jobs,youth,overall}_{5,15}km_min) filter the
--     precomputed polygon-edge proximity columns as plain `>= value`.
-- The city_index_rules + city_proximity (legacy centroid) paths are unchanged.
--
-- Adding params changes the signature, so DROP + CREATE (CREATE OR REPLACE
-- can't add arguments). The eight new params default NULL.
