-- 080_browse_stats_city_quality.sql
--
-- Phase QUAL Stats-tab parity. The Browse Map / Table / Cards paths
-- already honour the city-quality filter through the separate
-- `listings_with_city_quality` RPC + PostgREST `.in('sreality_id', ...)`
-- prefilter. The Stats tab calls `browse_stats` directly, so until this
-- migration its counts ignored the city-quality clauses — Stats showed
-- a wider cohort than Map/Table whenever a city-quality rule was set.
--
-- This migration extends the existing 40-param `browse_stats` to 44
-- params, adding the same four city-quality inputs the listings RPC
-- accepts:
--   city_index_rules jsonb     — array of {index_name, value}, AND'd.
--   city_pop_min int           — listing must be inside a curated
--   city_pop_max int             city with population in [min, max].
--   city_proximity jsonb       — {index_rules, population_min, radius_km}
--                                 — listing must be within radius_km
--                                 of a city matching the inner rules.
--
-- Predicates mirror `listings_with_city_quality` exactly so Stats and
-- the listings query never disagree on what a rule means. The fast
-- path is preserved: when every city-quality param is NULL/[], the
-- short-circuit `is null or jsonb_array_length(...) = 0` skips the
-- spatial subquery entirely.
--
-- Aliasing matters
-- ----------------
-- The previous `browse_stats` selected `from listings_public` (no
-- alias). The new city-quality EXISTS subqueries reference `lng` /
-- `lat` from the outer scope, but `curated_cities_public c` also has
-- columns named `lng` / `lat`. Without an alias on the outer select,
-- Postgres binds the bare `lng` / `lat` to the INNER scope (i.e. the
-- city's own coordinates), turning `ST_DWithin(city → city, radius)`
-- into a trivially-true predicate that ignores the listing entirely.
-- This migration aliases the outer select as `listings_public l` and
-- qualifies every column reference inside the new EXISTS blocks (and
-- every other column reference touched by the rewrite) with `l.` so
-- the resolution is unambiguous.
--
-- Same DROP-then-CREATE pattern migration 074 uses: adding params
-- creates a new overload, and named-argument callers (which we use)
-- become ambiguous against the prior 40-param version. The DROP
-- removes the old overload; grants come from the role-level
-- `GRANT EXECUTE ... TO anon` already on the database.
...
as $$
  with filtered as (
    ...
    where
      ...
      -- Phase QUAL: proximity block. Same alias discipline.
      and (
