-- 172_browse_stats_district_admin_id.sql
--
-- Make the Stats tab's District chip match by STABLE ADMIN ID at the picked
-- level, instead of fuzzy ILIKE across the name columns -- the same fix landing
-- on Map/Table (queries.ts) and Watchdog (api/notifications.py), so all four
-- agendas keep one definition of "matches" (architectural rule 16).
--
-- Two new parallel arrays accompany the existing districts_filter (names):
--   districts_levels text[]  -- 'obec' | 'okres' | 'kraj' | 'locality' | NULL
--   districts_ids    bigint[] -- admin_boundaries.id for an admin level, OR the
--                                containing obec_id for a 'locality' chip; NULL
--                                when the pick didn't resolve.
-- Per chip:
--   obec     -> l.obec_id   = id     (no more obec/okres name collision)
--   okres    -> l.okres_id  = id
--   kraj     -> l.region_id = id
--   locality -> (id is null or l.obec_id = id) and l.locality ilike *name*
--   else (legacy/unresolved, e.g. a pre-existing saved filter with no level) ->
--            the previous district/locality/okres/region ILIKE name (+ context)
-- A caller that passes only districts_filter (the deployed frontend, before the
-- new build ships, and every legacy saved filter) lands in the `else` branch ->
-- behaviour is byte-for-byte unchanged. New params at the END with DEFAULT NULL.
--
-- Reproduced VERBATIM from migration 165; the ONLY changes are (1) the two
-- trailing params, (2) the include-match and exclude-match unnests gain the
-- levels/ids columns, and (3) the per-chip predicate becomes level-aware. The
-- signature changes, so the current (165) overload is DROPped first.
