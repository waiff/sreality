-- 141_surface_okres_region.sql
--
-- Surface the geo-derived admin hierarchy (migration 140: listings.obec / okres
-- / region) on the public views, and make the existing location search match
-- okres + region as well as district + locality.
--
-- Today a search for a kraj ("Olomoucký kraj") or okres ("okres Olomouc") name
-- matches almost nothing, because those strings live in neither `district` nor
-- `locality`. Adding `okres` / `region` to the chip predicate makes region- and
-- okres-level location search work — additively (it can only ADD matches), and
-- consistently across Map / Table / Stats / Watchdog (the four places that
-- implement the per-chip "(name match) AND (optional context match)" predicate).
-- Nothing exact-matches these columns, so existing search is never narrowed.

-- --- listings_public: expose okres + region (trailing) -----------------------
...
-- --- properties_public: expose okres + region from the repr listing ----------
...
-- --- browse_stats_properties: broaden the district chip to okres + region ----
-- Reproduced verbatim from migration 133 (current prod def); the ONLY change is
-- the per-chip name-half and context-half each gain `l.okres` / `l.region`.
create or replace function public.browse_stats_properties(
  districts_filter text[] default null::text[], dispositions_filter text[] default null::text[],
  price_min_filter integer default null::integer, price_max_filter integer default null::integer,
  area_min_filter integer default null::integer, area_max_filter integer default null::integer,
  active_only_filter boolean default false, last_seen_min_days integer default null::integer,
  last_seen_max_days integer default null::integer, first_seen_min_days integer default null::integer,
  first_seen_max_days integer default null::integer, tom_days_min integer default null::integer,
  tom_days_max integer default null::integer, has_balcony_filter boolean default null::boolean,
  has_lift_filter boolean default null::boolean, has_parking_filter boolean default null::boolean,
  inactive_only_filter boolean default false, furnished_filter text default null::text,
  terrace_filter boolean default null::boolean, cellar_filter boolean default null::boolean,
  garage_filter boolean default null::boolean, category_sub_cb_filter integer default null::integer,
  building_type_filter text[] default null::text[], tag_ids bigint[] default null::bigint[],
  category_main_filter text default null::text, category_type_filter text default null::text,
  bbox_west double precision default null::double precision, bbox_south double precision default null::double precision,
  bbox_east double precision default null::double precision, bbox_north double precision default null::double precision,
  ownership_filter text default null::text, estate_area_min_filter double precision default null::double precision,
  estate_area_max_filter double precision default null::double precision, usable_area_min_filter double precision default null::double precision,
  usable_area_max_filter double precision default null::double precision, parking_lots_min_filter integer default null::integer,
  garden_area_min_filter double precision default null::double precision, garden_area_max_filter double precision default null::double precision,
  condition_match_filter text[] default null::text[], districts_context_filter text[] default null::text[],
  city_index_rules jsonb default null::jsonb, city_pop_min integer default null::integer,
  city_pop_max integer default null::integer, city_proximity jsonb default null::jsonb,
  price_per_m2_min double precision default null::double precision, price_per_m2_max double precision default null::double precision,
  distinct_site_count_min integer default null::integer, price_drop_count_min integer default null::integer,
  price_rise_count_min integer default null::integer, max_price_drop_pct_min double precision default null::double precision,
  portal_filter text[] default null::text[], mf_gross_yield_pct_min double precision default null::double precision,
    ...
    where
      ...
      and (
        ...
        or exists (
          ...
          where (l.district ilike '%' || needle || '%' or l.locality ilike '%' || needle || '%'
      ...
      and (((city_index_rules is null or jsonb_array_length(city_index_rules) = 0) and city_pop_min is null and city_pop_max is null)
        or (l.lat is not null and l.lng is not null and exists (
            ...
            where st_dwithin(st_setsrid(st_makepoint(l.lng, l.lat), 4326)::geography, st_setsrid(st_makepoint(c.lng, c.lat), 4326)::geography, c.default_radius_m)
              and (city_pop_min is null or c.population >= city_pop_min) and (city_pop_max is null or c.population <= city_pop_max)
