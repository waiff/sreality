-- 221: browse_stats_properties — make category_main a MULTISELECT.
--
-- The Browse "Type" pills (byt/dum/komercni/pozemek/ostatni) become multi-select
-- (registry filter `category_main_in`, mirroring the dispositions/disposition_main
-- split). The Stats RPC therefore needs `category_main_filter` widened from a scalar
-- `text` (= equality) to `text[]` (= any(...)), exactly like subtype/portal/etc.
-- already are. category_type stays SCALAR (rent vs sale are different price scales,
-- so the deal-type pill stays single-select).
--
-- A parameter TYPE change is NOT a body-only CREATE OR REPLACE: Postgres keys
-- functions by their argument types, so a bare CREATE would leave the old
-- `(... category_main_filter text ...)` overload in place and PostgREST would 404
-- (PGRST202) on the ambiguous call. We DROP the exact current 73-arg signature
-- first, then CREATE the new one. The body below is reproduced VERBATIM from
-- migration 203; the ONLY changes are (a) category_main_filter text -> text[] in the
-- signature and (b) its WHERE clause `= category_main_filter` -> the null/empty-safe
-- `= any(category_main_filter)` (the same shape subtype_filter already uses).
--
-- DEPLOY ORDER: apply this migration BEFORE (or together with) the frontend that
-- sends an array; the Stats tab 404s in the gap otherwise.
