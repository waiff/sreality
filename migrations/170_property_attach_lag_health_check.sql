-- 170_property_attach_lag_health_check.sql
--
-- Add the stage-2 latency check: time from a listing being scraped (written to
-- `listings`) to being Browse-visible. A new listing lands with property_id NULL
-- and is invisible in Browse (which reads the property grain, properties_public)
-- until the async property-maintenance job (recompute_property_stats --incremental,
-- ~every 5 min; daily full sweep as backstop) attaches it as a singleton. The
-- existing detail_queue_lag check covers stage 1 (index-seen → detail-fetched);
-- this closes the end-to-end picture (sreality-posted → Browse-visible).
--
-- listings_public does NOT expose property_id, and scraper_health_checks is
-- SECURITY INVOKER (anon), so it can't read the unattached state from the public
-- views. A tiny SECURITY DEFINER helper returns only the aggregate (count + oldest
-- age) per source — no row data leaks. ADDITIVE: one new function + one new check
-- layered onto the current (migration 169) function body.

create or replace function public.unattached_listings_stat(p_source text)
 returns table(n int, oldest_min numeric)
 language sql
 stable
 security definer
...
grant execute on function public.unattached_listings_stat(text) to anon, authenticated;

-- scraper_health_checks: current (migration 169) body + an `attach` CTE and a
-- property_attach_lag check (placed right after detail_queue_lag so the two
-- pipeline-latency rows sit together).
...
from calc, recon, queue, cad, lag, attach;
$function$;
