-- 103_region_stats_properties.sql
-- PR6 of the multi-portal dedup completion: move the Region page's two RPCs to
-- the deduplicated property grain. They previously read listings_public (raw
-- per-source rows); on properties_public each real-world property counts once,
-- so once a second portal lands, region counts / medians / TOM stop
-- double-counting the same property listed on multiple sites.
--
-- These read the EXISTING properties_public rollup view (maintained by the
-- recompute job) — NOT a new aggregation path — so they stay forward-compatible
-- with the scaling track's dirty-set recompute. Bodies are verbatim copies of
-- the latest definitions (region_stats from migration 024, region_active_by_day
-- from 012) with the only change being `from listings_public` ->
-- `from properties_public`. Signatures / return types / grants are unchanged,
-- so CREATE OR REPLACE swaps the body in place and existing callers are
-- unaffected. Identical results today (properties are 1:1 with listings).
