-- 063_browse_stats_price_band_velocity.sql
--
-- Replace the four-equal-bucket `price_quartile_velocity` output with
-- a seven-band percentile split. The operator asked for the y-axis
-- to be (p0–p10, p10–p25, p25–p45, p45–p55, p55–p75, p75–p90,
-- p90–p100) — narrow bands at the tails and median, wider bands
-- through the body — so the chart can surface tail-vs-body
-- differences (e.g. the top decile sitting noticeably longer on
-- market) that ntile(4)'s coarser cuts mask.
--
-- The new field is `price_band_velocity`. Each row carries the
-- percentile range that defines its membership (`p_lo`/`p_hi`), the
-- observed price range (`price_min`/`price_max`), the actual count
-- and share-of-cohort (because ties at percentile boundaries make
-- bucket sizes drift from their nominal width), and the same
-- tom_box (min, p25, median, mean, p75, max) shape that was used
-- by the prior `price_quartile_velocity` output.
--
-- Pure CREATE OR REPLACE: signature is unchanged from 062, so no
-- DROP and no re-grant. The old `price_quartile_velocity` key is
-- removed in lockstep — the frontend is updated in the same PR.
