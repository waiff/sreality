-- 032_skill_rental_estimator_full.sql
--
-- Phase 7 slice 1.5: expose the dormant phase 3b / 4b / 6 tools to
-- the reasoning agent by seeding a richer skill. The slice‑1 skill
-- `rental_estimator_v1` keeps the curated five-tool subset for
-- runs that prefer the cheap deterministic ladder; this new skill
-- adds the seven additional toolkit functions that were already
-- built but never reachable from the loop:
--
--   - compute_market_velocity, compute_listing_velocity  (phase 3b)
--   - compute_walkability, compute_amenity_supply,
--     find_comparables_along_axis                         (phase 4b)
--   - summarize_listing, compare_listing_images           (phase 6)
--
-- The agent decides when to reach for them; the system prompt body
-- below (canonical copy in skills/rental_estimator_full_v1/SKILL.md)
-- describes the gating rules.
--
-- No schema changes — `skills` already exists from migration 029.
-- This migration is one INSERT.

insert into skills (
  name, description, system_prompt,
  allowed_tools, preferred_model, limits, updated_by
) values (
  'rental_estimator_full_v1',
  'Czech apartment rental estimator with full toolkit (velocity, walkability, transit corridor, visual). Defaults to byt / pronajem.',
