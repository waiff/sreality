-- 378_browse_stats_property_ids_filter.sql
-- Stats must honour the deal-pipeline scope (rule #22).
--
-- Cards / Table / Map / the header count all narrow by an id allowlist through
-- one shared path (queries.ts resolveBrowsePrefilters → .in('property_id', …)).
-- Stats is the one Browse surface that is an RPC instead of a PostgREST query,
-- so it cannot receive that `.in(...)`; without a parameter of its own it would
-- keep aggregating the WHOLE market while the list beside it shows only the
-- operator's pipeline. That count-vs-list divergence is the exact failure
-- migration 351 was written to close, so the parameter ships with the filter.
--
-- `property_ids_filter` is deliberately GENERIC rather than a `pipeline_only`
-- flag: it is the seam for any future property-grain prefilter (the collection
-- membership filter is the obvious next one), and it keeps the account-scoped
-- pipeline join OUT of a market-data function — the client resolves the
-- allowlist under its own RLS, exactly as the market-growth obec allowlist has
-- done since migration 162.
--
-- Body reproduced VERBATIM from migration 375 (the city-quality read-model
-- rewrite — verified byte-identical to the live function before this change).
-- The ONLY edits are (1) the trailing parameter and (2) the single WHERE clause
-- beside `obec_ids_filter`. The signature changes, so the exact current
-- overload is DROPped first — otherwise PostgREST sees two candidates and
-- resolves ambiguously.
--
-- ORDERING: this file must be applied AFTER migration 375, whose body it
-- carries forward. Applying it against a database that lacks 375's
-- `browse_list.home_city_id` fails loudly (undefined column) rather than
-- silently reverting that work.
...
drop function if exists public.browse_stats_properties(
  ...
  integer, double precision, boolean, boolean
);
...
-- public-release hardening). CREATE hands EXECUTE back to PUBLIC by default, so
-- restore the exact ACL — without this, anon regains the Stats RPC.
