-- 549_browse_aggregates_know_ownership_jine.sql
-- FIELD CAPTURE W5 -- the Browse aggregates learn the fourth ownership value.
--
-- THE DEFECT. W5 adds `jine` to `toolkit/filter_registry.OWNERSHIP_CANONICAL`
-- (a stated fact five portals publish: idnes "jine"/"s.r.o."/"podilove",
-- mmreality "Jine", bezrealitky "OSTATNI", ceskereality's "statni, obecni,
-- jine", realitymix's own slug). The `__unknown__` ownership pill means "NULL
-- or not a canonical value", and the Browse LIST computes that from the
-- registry -- so the moment `jine` is canonical, the list stops counting those
-- rows as unknown. These two aggregate RPCs hardcode the OLD three-member
-- array, so Stats and the map keep counting them as unknown: the same rule-16
-- split migration 547 closed on the plot measure, one predicate over.
--
-- Measured 2026-09-22 (read-only): `ownership = 'jine'` is 73 active / 209
-- inactive rows today. It grows once the four portals that used to DROP their
-- "other" bucket start writing it.
--
-- ADDITIVE in effect: one array literal per function, one element longer. No
-- parameter changes, so CREATE OR REPLACE is enough and the ACL is untouched.
--
-- The bodies below are migration 547's, verbatim except those two literals,
-- verified byte-identical to the live `prosrc` before this file was generated
-- (md5 822e8fad4616550a5dd1b4be376c0014 for browse_stats_properties,
-- 180581f2d19fd898094c81db1fbc0fae for browse_map_cells).
--
-- DRIFT GATE -- RUN THIS IMMEDIATELY BEFORE APPLYING, not only when the file
-- was written. This is a whole-body CREATE OR REPLACE, so a parallel branch
-- that redefined either function first would be silently REVERTED by it (same
-- parameter list, no error, no ACL change). These two are the repo's most
-- rewritten RPCs -- 504 -> 537 -> 547 -> 549 in two months, four programs:
--
...
-- the same two-literal swap) instead of applying it as written.
--
-- APPLY BEFORE MERGING, for the same reason 547 did: the SPA shipping with this
-- file already treats `jine` as canonical, so until this lands the list and the
-- aggregates beside it disagree by those 73 rows.
