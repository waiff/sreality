...
- [x] **Dotted thousands in the one area grammar (same PR).** `scraper.area` read "1.910 m2" as 1,91:
      121 active area-less rows carry the form (bazos 69, remax 27, realitymix 20, idnes 5) and bazos
      holds 313 active parcels under 5 m². `AREA_NUMBER_SRC` accepts the form on every portal at once and
      the lane's figure check is built from it (one number shape, rule 21).
- [ ] **Heal the dotted-thousands rows after merge**: `reparse.yml` `fields=area_m2` on bazos, remax,
      realitymix, idnes (dry run first). (A wider LLM unit rule was measured and is NOT worth it: the
      model's misses are the wrong measure, not the unit — PROGRAM.md § 8.)
