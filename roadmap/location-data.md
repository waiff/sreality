...
| W4 targeted refetch cohorts | sreality legacy-shape + truncated refetch, bezrealitky remainder | 🟡 **MEASURED + RECONCILED (2026-09-08); refetch dispatch awaits the operator** — consumer (#1083) + lane (#1108) shipped 08-18/20, then parked while W2 closed. **First runs 2026-09-08**: the 38,612-row cohort measured as **32,053 delisted + 614 already post-cutover (the stale high-water mark, now measured) + 5,945 real refetches + 0 exhausted**; `mode=reconcile` retired the first two. The 5,945 are NOT yet enqueued: `mode=dispatch` commits ~6k sreality detail fetches (at `priority = -1`, behind live discovery, through the ordinary drain) and is the operator's dispatch — one command, `gh workflow run location_refetch_cohort.yml -f mode=dispatch -f limit=6000`. **Gate baseline (sampled, TABLESAMPLE 8 %): legacy-shape share of ACTIVE sreality rows = 5.76 %** (target < 2 %). W4(b) bezrealitky: 133 pre-0m-shape rows to refetch; the ≥ 95 % `ruianId` gate is **portal-inventory-capped at 48.08 %** (2,957 of 5,951 active publish `null`), exactly as W1v recorded. W4(c) truncated cohort is **empty**. See the W4 section |
