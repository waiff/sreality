...
- 2026-09-10 (a) — **Gate 1 CLOSED. Wave 2 opened on the operator's PATH C ruling. PR 1 of
  ...
  **What the brief left to be defined, and how PR 1 defines it (the operator asked for "the 'if
  ...
  5. **Floor for the town key**: obec, any confidence, `obec_kod` present — the contract said the
     rung "needs its own operator ruling"; it now has one. It was recorded as a `dedup_path_c` row
     in `location_data/serving_contracts.py` until W3 S4 deleted that module (fourteen declared
     floors, zero production readers); the floor now lives in the path definition itself,
     `toolkit/dedup_candidates.PATHS["C"].block_key = "obec_kod"`, which cannot block finer than
     the column it blocks on.
...
- 2026-09-10 — **The location engine is readable; read it, not the legacy columns** (recorded
  ...
  There is no flag to flip: W2-b deleted the per-feature switch module and W3 S4 the floor
  table, so cutting this program's reader over is a PR, gated by its own 7-day shadow compare.
