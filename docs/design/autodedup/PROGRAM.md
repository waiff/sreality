...
Every rule is stated **once**, here or in the section that owns it, and referenced by number everywhere else. **Never renumber E1–E48** — the numbers are cited by code, tests and the other program docs. A rule that changes meaning gets a new number and the old one is marked superseded.
...
- `POST /autodedup/verdict` `{kind, cluster_key|lo+hi, verdict, note}` — a cluster `same` also retracts every operator veto inside the group (E52)
- `POST /autodedup/verdict/split` `{cluster_key, generation, units:[{listing_id, unit}], relation, relations?, confirm_retract?, note}` — one group ruled unit by unit, a relation per unit pair (E51); the pairwise fan-out, the 409 confirmation and the must-not-link semantics are in §9
...
- **Numbers are cited by code and tests — never renumber E1–E49.** A rule that changes meaning gets a new number and the old one is marked superseded.
