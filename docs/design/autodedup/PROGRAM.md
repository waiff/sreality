...
**The split verdict** — `POST /autodedup/verdict/split` `{cluster_key, generation, units:[{listing_id, unit}], relation, note?}` — is how a whole group is ruled on unit by unit, because a group is frequently not one answer. The operator labels every member with a unit (a letter), and the route writes, **in one transaction**:
...
- for each member pair **across two units**: a pair verdict equal to `relation` (one of `same_building_different_unit` · `same_project_different_unit` · `different` — `same` is not offered, since two different units are never one property), plus a permanent `must_not_link` with reason `operator split: <relation>`;
- the **cluster** verdict: `same` when one unit was used, otherwise `relation`, carrying the assignment (`A: 94020,140903 | B: 94492 | C: 268009`) as its note.
...
- `POST /autodedup/verdict/split` `{cluster_key, generation, units:[{listing_id, unit}], relation, note}` — one group ruled unit by unit; the pairwise fan-out and the must-not-link semantics are in §9
