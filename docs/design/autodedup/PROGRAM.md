...
| M8 | band width `b` | 0.015 | **0.2023** at the W4 row (7,291 of 36,047 stored pairs *land in* the band; score interval 0.3215) — up from the hand prior's 0.0596 because E45–E48 demote rather than merge. `t_lo` is the dial and the curve is in `settings/w4_strata.json` | `scratchpad/eval_run2` | W4 |
...
| M14 | auto-merge precision (Wilson LB, n≥380) | ≥0.99 target (D3) | point **99.66 %** unweighted / **99.46 %** HT on 292 judged auto-merges; **Wilson LB 98.09 %**, bootstrap LB 98.38 %; sealed holdout 22/22 = **100 %**, CP LB 84.56 %. **The point gate is met, the LB gate is not — and cannot be at n=292 (381 needed)** | `scratchpad/eval_run2` | W4 (re-read at W6) |
| M15 | missed-duplicate rate per listing | ≤2% target | zone-based on weighted judged positives: **53.20 % auto-merge, 42.51 % band, 4.29 % reject** (the score-only line reads 3.53 % below `T_lo` and is blind to the gates). Sealed split: **8.73 %** missed at the shipped `T_lo`, 0 % at the store floor — the gap is one tie group of high-weight low-scoring positives. Six judged positives (weight 159 of 20,537) score below `store_floor` and are never stored at all; counting them the figures are 52.79 / 42.18 / 5.03 % | `scratchpad/eval_run2` | W4 (per-listing form still owed at W6) |
