...
1. **Production merges.** Everything so far writes only to the shadow tables. Going live means the engine's merge groups write through the existing merge function, which already carries your notes, tags, collections and pipeline cards onto the survivor and can be undone group by group. *Recommendation: start with sales in the trial area, watch the review pages for a week, then widen.* **Built (A1), dark:** the lane mode `apply` (`gh workflow run autodedup.yml -f mode=apply -f args=generation=<g>`) plans a generation's merges and changes nothing; `dry_run=0` merges only while `autodedup_apply_enabled` is true on /settings (seeded off) and only inside `autodedup_apply_scope` (seeded to sales, with no area until you name one), and `mode=unapply` (`dry_run=0`) undoes a generation group by group, newest first — after which no group of that generation merges again unless you dispatch `apply` with `reapply=1`.
