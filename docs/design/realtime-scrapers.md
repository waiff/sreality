...
**Autodedup lane — THE one path of the dedup engine (W5, E908–E915; ships DARK):** one bounded
pass of `autodedup.incremental_lane.run_incremental` — its ONLY caller (the GitHub `*/10` schedule,
...
- **It decides, groups AND merges.** A pass claims what moved, decides it with the batch engine's
  ...
  = must-link, `different` = must-not-link), commits the `rt` generation inside schema `autodedup`,
  and then RECONCILES production under the same lease (`autodedup/reconcile.py`, A9): the groups it
...
- **The engine bounds its own time (E913).** A pass reads its deadline (1,050 s) between steps and
  ...
  size. The claim is the engine's: its time budget (half the deadline) × the rate it measured. The
