...
| **W7 — SHIPPED** | The text lane on the realtime worker; the bake-off harness; per-field gates | the old cache key; the `confidence` enum; `run_vision_batch`'s private loop; the extract-but-don't-write mode and its backlog arm | the estimate of ~170 was the LANE alone and it is ~90; the branch is +2,782 / −90 over 20 files, of which the bake-off harness, the clear script, the migration and 642 test lines are the rest | W0, W2, W3, W6 |
...
- ✅ **bezrealitky `ruianId` rung — the evidence says FEED IT.** 2,836 of 5,716 active bezrealitky rows carry a
  ...
  **Shipped, and the item was one artefact short of the whole cost.** Declaring the type is not a YAML-only act:
  `contracts.CLAIM_TYPES` did not carry `address_point_id`, so the contract loader refused it before the goldens
  could ever be reached. W1-c cut the type as COLLATERAL — bezrealitky@1 was its only carrier and that contract was
  being slimmed to one entry per type — and not on the cut's own test, "a type no resolver reads": `bind.py`'s R0
  reads it and scores it **100**, the highest base in the ladder. So the vocabulary is **twelve** now, with that
  reasoning written beside it and pinned in `test_claims_intake_contracts` (the count is a ceiling; a thirteenth
  needs the same evidence). `operator_corrections.py` had accepted the type throughout, which is why the corpus's
  one claim is an operator correction rather than a contract's.

  **Verified before the change (SQL, 2026-09-22):** 5,706 active rows / 2,827 with a `ruianId`, all digits;
  **2,819 (99.7 %) join a LIVE `ruian_address_points` row** on `kod_adm` with `valid_to IS NULL`, and the 8 that
  do not are absent from the mirror entirely (not superseded) — so the portal's id space IS `kod_adm`. Of the 750
  rows below `address_point`, **745 join** and the joined point's obec agrees with the obec the resolver already
  reached on **742**. The bump is `bezrealitky@3 → @4`: ONE appended entry (`bzr.det.ruian_id`, version 1's id,
  `scalar` on `/ruianId`, bare — R0 parses it with `int()`, so `namespaced_id` would break it), eight entries
  byte-for-byte unchanged, lockfile + `bezrealitky@4.json` regenerated by their own tooling, **no migration**.
  W18-b is what makes it safe: @4 declares every @3 type plus one, so the per-CLAIM-TYPE version rail keeps each
  existing type on its @3 claims until re-mined, and a type with no claim at any version contributes nothing.
  **The fill is not automatic** — the bodies-half hash gate re-arms PAGE entries and bezrealitky has none, so the
  corpus-wide mine needs one dispatched `location_claims_intake.yml` at `mode=full source=bezrealitky`.
