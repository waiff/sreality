...
| **W7 — SHIPPED** | The text lane on the realtime worker; the bake-off harness; per-field gates | the old cache key; the `confidence` enum; `run_vision_batch`'s private loop; the extract-but-don't-write mode and its backlog arm | the estimate of ~170 was the LANE alone and it is ~90; the branch is +2,783 / −91 over 20 files, of which the bake-off harness, the clear script, the migration and 569 test lines are the rest | W0, W2, W3, W6 |
