...
- **Location-resolve lane** (Decision 8b) — THE resolver drain from here, not only from the
  ...
  SKIP LOCKED) — the heartbeat's `workers`/`failed_passes` say what actually ran. The GH lane
  stays single-connection. Exclusion, budgets, lease/lock: `docs/design/realtime-scrapers.md`. (The `epoch_job` it
