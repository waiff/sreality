...
- a **rung** is which attributes were compared: **C1** = town + disposition + area (both sides
  ...
  **C3** = town + area (taken when a disposition is missing on either side — the fallback is on
  ABSENCE, never on a mismatch). C2 does not exist. **Land never reaches C1**: the identity
  attributes are per category (`dedup_candidates.IDENTITY_ATTRS` — `pozemek`: area only), so a
  parcel arrives at the rule with no disposition whatever the portal printed on it.
...
- C1: equal disposition (trimmed; NULL/blank/not an identity attribute of the category = not
  available), plus the areas within `l0_c1_area_tolerance_pct` when both sides state one —
  skipped, pair kept, when either does not;
- C3: both areas present and > 0, the gap as a percent of the larger side ≤
  `l0_area_tolerance_pct_general` (or `_pozemek` when either side is land);
- **the area is `listings.area_m2`, for every category** (W17, generator version `c4`) — THE
  headline area, which `scraper/area.derive_headline_area` already resolves to the plot on land
  and the interior measure otherwise. The lane spells no area choice of its own; it used to
  (`estate_area` for pozemek, else `usable_area`), and that second definition disagreed with the
  headline on every land row the parsers left without one and on every dwelling measured only by
  a floor/total label;
