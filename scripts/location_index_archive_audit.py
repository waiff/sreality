...
INDEX_ARCHIVERS: dict[str, CallSite] = {
    "sreality": CallSite(
        module="main.py",
        state=STATE_GATED,
        call_site="scraper/main.py:1454 (_index_page_archiver.archive)",
        gate="main.py:1451 `if key in fresh:` returning on the next line — the set is "
             "db.fresh_index_page_keys(hours=INDEX_ARCHIVE_REFRESH_HOURS), read before the upsert",
        intentional_skips=(
            "main.py:963 `def probe_category(` fetches index pages via fetch_index_page and "
