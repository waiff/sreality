...
NAMING — READ THIS BEFORE ADDING A SIBLING LANE
  ...
  substrates: W3 (`location_data.claims_remine`) reads `listing_snapshots.raw_json`;
  this one reads archived page bodies. `location_claim_batches` resume/watermark is keyed
  on `(lane, source, scan_mode)`, so one shared lane string would have had the two cursors
  overwriting each other the first time both ran — a silent coverage hole, not a crash.
  W3 shipped first and kept the short names; everything here is disambiguated:
  module `claims_remine_archive`, `LANE = "location_claims_remine_archive"`,
  `REMINE_VERSION = "claims_remine_archive@1"`. The dispatch workflow W2-13 will add takes
  the same treatment — `location_claims_remine_archive.yml`, inner concurrency group
  `location-remine-archive` — so the collision is not recreated one layer out.
...
THE THREE RULINGS THIS LANE IMPLEMENTS (BUILD-PLAN §1)
  ...
  * C4/06 §6.6 Rule 2 — `snapshot_id` NULL + `snapshot_anchor='unanchored_latest_fetch'`.
    That is the only anchor `loc_claim_anchor` permits beside a NULL `snapshot_id` in this
    lane's sense, and forcing a snapshot onto an archived body would assert an anchoring
