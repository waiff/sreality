...
The collision is not cosmetic. These strings are PRIMARY KEYS in the coordination tables:
  ...
  * `CONCURRENCY_GROUP` is the outer serialisation group (00 §14); sharing one is sometimes
    DELIBERATE (the location-batch rails), so it is checked per-module, not per-group.
...
# share a namespace when they land in the SAME column: `INTAKE_VERSION` and
# `REMINE_VERSION` are both `location_claim_batches.extractor_version`, so `claims_remine@1`
# and `claims_intake@3` are competing for one identity space despite the different spelling.
...
_KNOWN_LANES = frozenset({
    "location_claims_intake", "location_claims_remine", "location_claims_remine_archive",
})
_KNOWN_JOB_NAMES = frozenset({
    ...
    "location_payload_refetch_probe",
})
