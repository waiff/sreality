...
def test_parse_sreality_url_returns_spec_and_metadata() -> None:
    ...
    res = url_parser.parse_sreality_url(
        "https://www.sreality.cz/detail/prodej/byt/3+kk/praha-smichov-na-cisarce/3386655564",
        client=client,
        conn=conn,
    )
    assert res["sreality_id"] == 3386655564
    assert client.calls == [3386655564]
    assert res["source_url"].endswith("3386655564")
    assert "fetched_at" in res
    assert res["in_database"] is False
    spec = res["spec"]
    assert spec["sreality_id"] == 3386655564
