...
def test_dismiss_refuses_a_property_in_the_pipeline():
    conn = _FakeConn([
        (lambda q: "RECURSIVE chain" in q, [(42, 42)]),
        (lambda q: "FROM property_pipeline WHERE property_id" in q, [(1,)]),
    ])
    with pytest.raises(fastapi.HTTPException) as ei:
        dismissals_module.dismiss(conn, s.DismissPropertyIn(property_id=42), account_id=_ACCT)
    assert ei.value.status_code == 409
    assert _inserts(conn) == []
    # the pipeline check is RLS-scoped: it binds only the property
