...
one rollout control, the scope row, seeded with no area (OFF).
...
index is the one E905 names, and the seeded scope cannot drive a live run on its own.
"""
...
def test_the_source_check_is_widened_to_the_three_sources() -> None:
    ...
    # 475 stamped 'legacy' on every row of 2026-09-05, operator merges included: a date, not
    # an engine. Nothing here writes the column.
    assert "the pre-2026-08 engine" not in code and "update public.property_merge_events" \
        not in code
...
def test_the_scope_is_the_one_row_seeded_and_never_overwrites_the_operator() -> None:
    code = _code()
    assert "insert into app_settings (key, value, description)" in code
    assert "on conflict (key) do nothing;" in code
    assert re.findall(r"'(autodedup_apply_\w+)'", code) == [A.SCOPE_SETTING]
...
def test_the_seeded_scope_parses_and_cannot_drive_a_live_run() -> None:
    ...
    with pytest.raises(ValueError, match="area"):
        A.effective_scope(seeded, {"blocks": "town:563510"}, live=True)
