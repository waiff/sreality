...
| Sold comps | 🟡 active (W3) | Registered sales as their own account-less fact (`sold_transactions`, migration 542) — reas.cz only, per-obec cells, never a listing row and never mixed with asking prices. W0 + W1 shipped (shared table primitives; store + pure parser). **W3 built, migration 545 NOT yet applied**: the read surface, `Agenda.SOLD`, the listing-page block, and the reas.cz chip deleted | [roadmap/sold-comps.md](roadmap/sold-comps.md) |
