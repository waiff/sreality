...
| Sold comps | 🟡 active (W5) | Registered sales as their own account-less fact (`sold_transactions`, migration 542) — reas.cz only, per-obec cells, never a listing row and never mixed with asking prices. W0–W3 shipped (table primitives; store + pure parser; fetch path; read surface, `Agenda.SOLD`, listing-page block, reas chip deleted). **W5 shipped**: the subtraction payment — `FilterChip` and `POST /tools/find_comparables` deleted; `category_sub_cb` refused with reasons. **Remaining: migration 545 NOT yet applied, and the operator turns the lane on** | [roadmap/sold-comps.md](roadmap/sold-comps.md) |
