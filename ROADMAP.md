...
| Sold comps | 🟡 active (W5) | Registered sales as their own account-less fact (`sold_transactions`, migration 542) — reas.cz only, per-obec cells, never a listing row and never mixed with asking prices. W0–W2 shipped (table primitives; store + pure parser; fetch path + lane). **W3 built, migration 545 NOT yet applied**: the read surface, `Agenda.SOLD`, the listing-page block, and the reas.cz chip deleted. **W5 built**: the subtraction payment — `FilterChip` deleted; `POST /tools/find_comparables` deleted **pending the operator's "nothing outside the repo calls it"** (merge gate, `roadmap/sold-comps.md`); `category_sub_cb` refused with reasons. **Remaining: migration 545, that merge gate, and the operator turns the lane on** | [roadmap/sold-comps.md](roadmap/sold-comps.md) |
