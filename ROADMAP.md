...
| Field capture | ✅ delivered 2026-09-22 | Every typed listing fact: ONE declared producer per (portal, field), ONE canonical vocabulary, a MEASURED fill rate **and** accuracy — stated facts parsed at ingest, prose-only facts extracted after publication. W0–W9 shipped (#1559–#1592, migrations 546–556): contract table + CI gates, one re-parse seam, structured gaps healed on all nine portals, one vocabulary, preserve-if-null, floor ground = 0 everywhere, and the text lane on the realtime worker extracting bazos `floor` + `has_lift` on the bake-off winner gpt-5.6-luna. Open gates widen by evidence only (R7). Rulings R1–R12 + gates: `docs/design/field-capture/PROGRAM.md` | [roadmap/field-capture.md](roadmap/field-capture.md) |
