...
| Sold comps | 🟢 shipped, lane dark | Registered sales as their own account-less fact (`sold_transactions`, migs 542–545) — reas.cz only, per-obec cells, never a listing row and never mixed with asking prices. W0–W3 shipped: shared table primitives, store + pure parser, the fetch lane (dark behind `realtime_sold_comps_interval_seconds`), the read surface + `Agenda.SOLD` + the listing-page block; the reas.cz chip deleted. **Next: the operator turns the lane on.** Parked: `POST /tools/find_comparables` deletion awaits an operator answer | [roadmap/sold-comps.md](roadmap/sold-comps.md) |
