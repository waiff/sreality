...
| NEW DEDUP | 🟡 active (W2) | **W0 DONE — Gate 0 closed 2026-09-05**; **W1 DONE — Gate 1 closed 2026-09-09** (training set finalized, tag model v1 live; accuracy keeps iterating in parallel). **W2 in progress — Level 0 candidate selection, path C first** (town = `obec_kod` + disposition / area; ruled 2026-09-10): all three PRs landed — store + settings (migration 492 written, **not applied**), the generation lane (**not run**) and the audit page `/new-dedup/candidates`; **Gate 2 open** until an estimate is read, 492 is applied and a generate runs — see `docs/design/new-dedup/PROGRAM.md` | [roadmap/new-dedup.md](roadmap/new-dedup.md) |
| Location data | 🟡 active (**simplification sprint BUILT 2026-09-12**, W0–W5 — one store `listing_location`, one hourly claim lane, eleven claim types, one label function, one four-level code predicate, one consumer rule; rules 24+25 folded into one. AWAITING: the town-coverage gate (`check_location_town_coverage` red until zero), and the operator applying migration 514) | Location SSOT: portal claims → a four-step resolver (bind → fill → grade → check) → ONE answer table on RÚIAN (`listing_location`, 26 cols), graded precision, licence enforced at the claim read. `listings`/`properties` carry no place column; every display is `location_display_label`, every filter four levels of RÚIAN codes. Red line: every listing has a row, every Czech listing has a town (W15: the lane covers the whole database; the served set retired) | [roadmap/location-data.md](roadmap/location-data.md) |
...
| Measure unification | ✅ shipped (W1–W9; rail live) | One measure, one definition, one label — 64 per-m² sites across 4 territories collapsed onto one named SQL measure + basis label (rule #23), with a **permanent rail**: required-arg signatures, an offline CI census over six source trees + every effective SQL definition, and `FilterDef.basis`. **Open (operator):** the two W2 backfill write passes, and the `browse_stats` drop. Charter: `docs/design/ppm2-measure-unification.md` | [roadmap/measure-track.md](roadmap/measure-track.md) |
| Operator workflow | 🟢 mostly done | Collections / tags / notes, deal pipeline, dismissals; U-ME (manual rental estimates) next | [roadmap/operator-workflow-track.md](roadmap/operator-workflow-track.md) |
...
| Sold comps | 🟢 shipped, lane on | Registered sales as their own account-less fact (`sold_transactions`, migs 542–545) — reas.cz only, per-obec cells, never a listing row and never mixed with asking prices. W0–W3 shipped: shared table primitives, store + pure parser, the fetch lane (`realtime_sold_comps_interval_seconds`, on at 3600 since 09-21), the read surface + `Agenda.SOLD` + the listing-page block; the reas.cz chip deleted. `POST /tools/find_comparables` stays (operator ruling) | [roadmap/sold-comps.md](roadmap/sold-comps.md) |
...
| Reliability | 🟡 active (W0, W3) | Applied guarantees + one voice for failure, after the 2026-08-26 six-portal ingest outage (migration 438 merged but unapplied for 29h, invisible because `scrape_runs.errors = 0`). **W0 approved 2026-08-27** — four fixes that each stop *discarding* information: poller cursor loss, honest crash accounting at the shared `portal_runner` seam, LLM check keyed on state not recency, graceful degradation of the acute health lane. **W3.1–W3.3 shipped 2026-08-31** — text-derived failure signatures + `ops_incidents` (migration 462) exiting through the existing `system_health` bell; external routing awaits the operator. W1/W2/W4 proposed pending sign-off on eight open questions (branch protection on `main` is off, so every CI rail is advisory); W5 (constraint-only catalog diff) named but not built — see `docs/design/reliability-program.md` | [roadmap/reliability-track.md](roadmap/reliability-track.md) |
| Field capture | 🟡 active (W0 done) | Every typed listing fact: ONE declared producer per (portal, field), ONE canonical vocabulary, a MEASURED fill rate **and** accuracy — stated facts parsed at ingest, prose-only facts extracted after publication. **W0 done** — the `sreality_id`-keyed description-enrichment lane deleted wholesale (it reached 0.4% of bazos while three LLM health checks read green), with its two 0-row batch tables (migration 546, destructive, **not yet applied**) and `check_llm_liveness`, whose threshold was sized on a producer that no longer exists. W1 (per-portal key census + fill/validity matrix) next. Rulings R1–R12 + numeric gates: `docs/design/field-capture/PROGRAM.md` | [roadmap/field-capture.md](roadmap/field-capture.md) |
