...
| Sold comps | 🟡 active (W2) | Registered sales as their own account-less fact (`sold_transactions`, migration 542) — reas.cz only, per-obec cells, never a listing row and never mixed with asking prices. **W1 shipped** (store + pure parser + the deletion of the false "delisted = transacted-price proxy" claim); **W2 in progress**: the fetch path — client, writer, CLI and a `sold_comps` worker lane that ships dark | [roadmap/sold-comps.md](roadmap/sold-comps.md) |
