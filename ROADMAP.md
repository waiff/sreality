...
| Location data | 🟡 active (W1/W1v/W2/W3 shipped, W2a closing, **W4 sreality arms PASS 2026-09-09** — legacy share 8.4 % → 0.03 %; one arm still red until the operator dispatches 136 bezrealitky rows; **W4 CLOSED 2026-09-10 (gate PASS)**; **W6-2 SHIPPED 2026-09-10**: the filters + map old-vs-new side-by-side admin page (`/location-compare`, Praha + Středočeský) for operator review before any rollout; A5 decided, all seven portals un-shadowed 09-09, sweeps + LLM pass running chained; **bottleneck = the resolve drain at 0.7 listings/s** (Decision 8 with the operator); **W2-a SHIPPED**: the resolver at four steps (bind → fill → grade → check) writing the 26-column `listing_location` (mig 501), ~3,700 lines + the policy tables / epoch / contradiction ledger / property projection deleted; **W2-b BUILT (mig 502)**: six readers cut over to `listing_location`, the labelled-sample + compare surfaces deleted whole, 21 relations + the compare pg_cron job dropped — merges only after `resolver:v4` re-resolves the corpus and each portal's `cz_no_town` line is back at its pre-W2 level) | Greenfield location SSOT (claims → a four-step resolver → one answer table on RÚIAN, graded precision, licence enforcement at the claim read); spine live in **shadow** (migs 380–389, 399–400, 402–408), nothing reads it until W6; **W3 history backfill SHIPPED 2026-08-19 — 1.63 M snapshots mined, 92,312 historical claims, ~10.85 M observations, all four gate arms pass; measured oscillation is small (804 of 327,113 listings ever changed a location value), which should temper any wave resting on location churn**; W2a archive measured + bounded, dual-write ON, backfill terminal; **W2 ACTIVATION SHIPPED 2026-09-05 — all seven portal contracts activated and SHADOWED in one wave (W2-6…W2-12), 43 entries inert→executable, mig 470 closing the four-method survivorship-policy gap; the re-mine lane can now mine and nothing it mines serves until the operator un-shadows each portal off the W2-13 gate report** | [roadmap/location-data.md](roadmap/location-data.md) |
