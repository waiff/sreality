...
| Location data | 🟡 active (**simplification sprint BUILT 2026-09-12**, W0–W5 — one store `listing_location`, one hourly claim lane, eleven claim types, one label function, one four-level code predicate, one consumer rule; rules 24+25 folded into one. AWAITING: the town-coverage gate (`check_location_town_coverage` red until zero), and the operator applying migrations 502/506/508/512 in order) | Location SSOT: portal claims → a four-step resolver (bind → fill → grade → check) → ONE answer table on RÚIAN (`listing_location`, 26 cols), graded precision, licence enforced at the claim read. `listings`/`properties` carry no place column; every display is `location_display_label`, every filter four levels of RÚIAN codes. Red line: every served listing has a row, every active Czech listing has a town | [roadmap/location-data.md](roadmap/location-data.md) |
